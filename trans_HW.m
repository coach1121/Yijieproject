...
% Set the target frequency range (100-3000Hz)
target_low_freq = 100;
target_high_freq = 3000;
...
for i = 1:length(file_list)
    ...
    target_low_pixel = round((target_low_freq / 3000) * spec_height); % Use 3000 as the reference for high frequency
    target_high_pixel = round((target_high_freq / 3000) * spec_height); % Use 3000 as the reference for high frequency
    
    % Check and adjust indices to ensure they do not exceed the image dimensions
    target_low_pixel = max(1, min(target_low_pixel, spec_height));
    target_high_pixel = max(1, min(target_high_pixel, spec_height));
